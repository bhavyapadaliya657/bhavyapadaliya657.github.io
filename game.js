...
// BFS: can dst be reached from src?
function isReachable(grid, src, dst) {
  const visited = Array.from({length:ROWS}, () => new Array(COLS).fill(false));
  const queue   = [{r:src.r, c:src.c}];
  visited[src.r][src.c] = true;
  while (queue.length) {
    const {r,c} = queue.shift();
    if (r === dst.r && c === dst.c) return true;
    for (const [dr,dc] of [[-1,0],[1,0],[0,-1],[0,1]]) {
      const nr = r+dr, nc = c+dc;
      if (nr>=0&&nr<ROWS&&nc>=0&&nc<COLS&&!visited[nr][nc]&&grid[nr][nc]!==TILE.WALL) {
        visited[nr][nc] = true;
...
// BFS flood fill — returns every cell reachable from start
function findReachableCells(grid, start) {
  const visited = Array.from({length:ROWS}, () => new Array(COLS).fill(false));
...
// Generate a random interior layout; retries until ≥55% interior is one connected region
function generateRandomGrid() {
  for (let attempt = 0; attempt < 25; attempt++) {
    ...
    for (let r = 0; r < ROWS; r++) {
      grid[r] = [];
      for (let c = 0; c < COLS; c++) {
        grid[r][c] = (r===0||r===ROWS-1||c===0||c===COLS-1) ? TILE.WALL : TILE.FLOOR;
      }
    }

    // Horizontal wall segments — each has a random gap so passage always exists
    const hCount = 2 + Math.floor(Math.random()*3);
    for (let s = 0; s < hCount; s++) {
      const r   = 2 + Math.floor(Math.random()*(ROWS-4));
      const c0  = 1 + Math.floor(Math.random()*(COLS-6));
      const len = 2 + Math.floor(Math.random()*5);
      const gap = Math.floor(Math.random()*len);
      for (let i = 0; i < len; i++) {
        const cc = c0+i;
        if (cc > 0 && cc < COLS-1 && i !== gap) grid[r][cc] = TILE.WALL;
      }
    }

    // Vertical wall segments — each has a random gap
    const vCount = 2 + Math.floor(Math.random()*3);
    for (let s = 0; s < vCount; s++) {
      const c   = 2 + Math.floor(Math.random()*(COLS-4));
      const r0  = 1 + Math.floor(Math.random()*(ROWS-4));
      const len = 2 + Math.floor(Math.random()*4);
      const gap = Math.floor(Math.random()*len);
      for (let i = 0; i < len; i++) {
        const rr = r0+i;
        if (rr > 0 && rr < ROWS-1 && i !== gap) grid[rr][c] = TILE.WALL;
      }
    }

    // Random pillar tiles
    const pillars = 3 + Math.floor(Math.random()*6);
    for (let p = 0; p < pillars; p++) {
      const r = 1 + Math.floor(Math.random()*(ROWS-2));
      const c = 1 + Math.floor(Math.random()*(COLS-2));
      if (r>0&&r<ROWS-1&&c>0&&c<COLS-1) grid[r][c] = TILE.WALL;
    }

    // Connectivity check: need ≥55% of interior reachable
    let seed = null;
    outer: for (let r=1;r<ROWS-1;r++)
      for (let c=1;c<COLS-1;c++)
        if (grid[r][c]===TILE.FLOOR) { seed={r,c}; break outer; }
    if (!seed) continue;

    const reachable    = findReachableCells(grid, seed);
    const totalInterior = (ROWS-2)*(COLS-2);
    if (reachable.length / totalInterior >= 0.55) return grid;
  }

  // Fallback: very open map with minimal obstacles
  const grid = [];
  for (let r=0;r<ROWS;r++) {
    grid[r]=[];
    for (let c=0;c<COLS;c++)
  ...
  [[3,4],[3,13],[8,4],[8,13],[5,8],[5,9]].forEach(([r,c]) => {
    if (r>0&&r<ROWS-1&&c>0&&c<COLS-1) grid[r][c]=TILE.WALL;
  });
  return grid;
}

// Choose random fire seed positions, kept ≥4 tiles away from excludes (player/exit)
function placeRandomFire(grid, excludes) {
  const count = 2 + Math.floor(Math.random()*3); // 2–4 seeds
  const candidates = floorCells(grid)
    .filter(fc => !excludes.some(e => e.r===fc.r && e.c===fc.c))
    .sort(() => Math.random()-0.5);
  const placed = [];
  for (const cell of candidates) {
    if (placed.length >= count) break;
    if (!excludes.some(e => manhattanDist(cell, e) < 4)) placed.push(cell);
  }
  // Relax distance constraint if not enough seeds were placed
  if (placed.length < 2) {
    for (const cell of candidates) {
      if (placed.length >= 2) break;
      if (!placed.some(p => p.r===cell.r && p.c===cell.c)) placed.push(cell);
    }
  }
  return placed;
}

// Win condition: player steps on the exit tile
function checkExitReached() {
  if (state.player.row === state.exitRow && state.player.col === state.exitCol) {
    endWin();
  }
}
...
// ═══════════════════════════════════════════
function initGame() {
  // Generate random playable map; retry until player→exit path is guaranteed
  let grid, playerCell, exitCell, fireSeeds;
  let mapValid = false;

  for (let attempt = 0; attempt < 30 && !mapValid; attempt++) {
    grid = generateRandomGrid();

    // Flood-fill the main connected floor region
    let seed = null;
    outer: for (let r=1;r<ROWS-1;r++)
      for (let c=1;c<COLS-1;c++)
        if (grid[r][c]===TILE.FLOOR) { seed={r,c}; break outer; }
    ...
    const region = findReachableCells(grid, seed);
    if (region.length < 20) continue;

    // Random player spawn (first ~40% of shuffled region = variety without being too predictable)
    const shuffled = [...region].sort(() => Math.random()-0.5);
    playerCell = shuffled[Math.floor(Math.random() * Math.max(1, Math.floor(shuffled.length*0.4)))];

    // Exit must be ≥8 Manhattan tiles away from player
    const farCells = region.filter(c => manhattanDist(c, playerCell) >= 8);
    if (!farCells.length) continue;
    exitCell = farCells[Math.floor(Math.random()*farCells.length)];

    // BFS confirm player can actually reach exit
    if (!isReachable(grid, playerCell, exitCell)) continue;

    // Random fire seeds that don't block anything critical
    fireSeeds = placeRandomFire(grid, [playerCell, exitCell]);
    mapValid  = true;
  }
  ...
  // Place exit tile (always open from start — reaching it wins)
  state.exitCol  = exitCell.c;
  state.exitRow  = exitCell.r;
  state.grid[exitCell.r][exitCell.c] = TILE.EXIT;
  state.exitOpen = true;
  ...
  // Seed fire on chosen random cells
  fireSeeds.forEach(({r,c}) => {
    if (state.grid[r][c] === TILE.FLOOR) state.fire[r][c] = 1.0;
  });
...
function tryMove(dt) {
  ...
  // Win if player stepped onto exit
  checkExitReached();
  if (state.phase !== 'playing') return; // exit wins mid-move
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      for (const [dr,dc] of neighbors8) {
        const nr=r+dr, nc=c+dc;
        if (nr<0||nr>=ROWS||nc<0||nc>=COLS) continue;
        if (state.grid[nr][nc]===TILE.WALL || state.grid[nr][nc]===TILE.EXIT) continue;
        if (state.fire[nr][nc]>0) continue;
...
function endGameover(reason='fire') {
  ...
  if (titleEl)  titleEl.textContent  = isTimeout ? "TIME'S UP"           : 'YOU WERE OVERTAKEN';
  if (reasonEl) reasonEl.textContent = isTimeout ? "You didn't reach the exit in time." : 'The flames consumed everything.';
...
function endWin() {
  ...
  // Score bonuses
  const timeBonus = Math.floor(state.timeLeft * 100);  // 100 pts per second remaining
  state.score += state.health * 1000;  // health bonus
  state.score += 5000;                 // escape bonus
  state.score += timeBonus;            // speed bonus
  sfxWin();

  const timeRemEl = document.getElementById('win-time-remaining');
  if (timeRemEl) timeRemEl.textContent = state.timeLeft.toFixed(1) + 's';
